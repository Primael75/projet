...
<!-- Le fichier navbar.php contient le code HTML pour la barre de navigation, ce qui permet de le réutiliser sur plusieurs pages sans duplication de code -->
<body>
   <main>
        <?php include 'hero_express.php'; ?>
        <?php
         require 'db.php';                /*le require au lieu de include permet de s'assurer que le fichier est bien inclus, sinon le script s'arrête  ce qui est une bonne pratique pour les fichiers essentiels*/
            $sql = "SELECT * FROM produits WHERE categorie_id NOT IN (4) AND (prix > 70) "; // Requête pour récupérer tous les produits mise dans la variable $sql parce que je peux l'utiliser plusieurs fois et que c'est plus lisible et je peux la modifier facilement si besoin
